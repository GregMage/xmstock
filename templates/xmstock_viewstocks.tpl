		...
		<li class="list-group-item d-flex justify-content-between align-items-center">
			<div>
				...
				<{if $stock.manage|default:false == true}>
					...
					<a href="<{$xoops_url}>/modules/xmstock/transfer.php?op=add&type=E&article_id=<{$article_id}>&amp;area_id=<{$stock.area_id}>" class="btn btn-light" title="<{$smarty.const._MA_XMSTOCK_TRANSFER_ENTRYINSTOCK}>"><span class="fa fa-sign-in"></span></a>
					<a href="<{$xoops_url}>/modules/xmstock/transfer.php?op=add&type=O&article_id=<{$article_id}>&amp;area_id=<{$stock.area_id}>" class="btn btn-light" title="<{$smarty.const._MA_XMSTOCK_TRANSFER_OUTOFSTOCK}>"><span class="fa fa-sign-out"></span></a>
					<a href="<{$xoops_url}>/modules/xmstock/transfer.php?op=add&type=T&article_id=<{$article_id}>&amp;area_id=<{$stock.area_id}>" class="btn btn-light" title="<{$smarty.const._MA_XMSTOCK_TRANSFER_TRANSFEROFSTOCK}>"><span class="fa fa-external-link"></span></a>
					<a href="<{$xoops_url}>/modules/xmstock/transfer.php?article_id=<{$article_id}>&amp;area_id=<{$stock.area_id}>" class="btn btn-light" title="<{$smarty.const._MA_XMSTOCK_TRANSFER_LIST}>"><span class="fa fa-search"></span></a>
				...
				<{if $stock.loan|default:false == true && $stock.amount != 0}>
					<{if $stock.manage|default:false == true}>
					<a href="<{$xoops_url}>/modules/xmstock/caddy.php?op=add&amp;article_id=<{$article_id}>&amp;area_id=<{$stock.area_id}>" class="btn btn-light" data-toggle="tooltip" data-placement="top" title="<{$smarty.const._MA_XMSTOCK_LOAN_USERSLIST}>: <{$stock.borrower|default:''}>"><span class="fa fa-shopping-cart"></span> <{$smarty.const._MA_XMSTOCK_LOAN}></a>
					<{else}>
